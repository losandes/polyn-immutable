...
bp=name}else{bp=blueprint(name,schema)}if(bp.err){throw bp.err}return{validate:function validate(input){var validationResult=bp.validate(input);if(validationResult.err){throw validationResult.err}return validationResult}}}/**
     ...
     */var _toObject=function toObject(that,options){var shallowClone=Object.assign({},that);var output={};var _removeFunctions$opti=_objectSpread({},{removeFunctions:false},options),removeFunctions=_removeFunctions$opti.removeFunctions;Object.keys(shallowClone).forEach(function(key){if(shallowClone[key]&&typeof shallowClone[key].toObject==="function"){output[key]=shallowClone[key].toObject(options)}else if(is.array(shallowClone[key])){output[key]=Object.assign([],shallowClone[key])}else if(is.object(shallowClone[key])){output[key]=Object.assign({},shallowClone[key])}else if(is.function(shallowClone[key])&&removeFunctions===true){// do nothing
       ...
       * @param {object} schema - the blueprint schema
       */return function(name,schema,options){var validator=new config.Validator(name,schema);var _functionsOnPrototype=_objectSpread({},{functionsOnPrototype:false},options),functionsOnPrototype=_functionsOnPrototype.functionsOnPrototype;// NOTE the classes, and freezeArray are in here, so their
...
/**
         ...
         */var freezeArray=function freezeArray(input){return Object.freeze(input.map(function(val){if(is.array(val)){return freezeArray(val)}else if(is.object(val)){return new Immutable(val)}else{return val}}))};/**
         * Freezes an object, and all of it's values, recursively
         * @param {object} input - the object to freeze
         */var Immutable=/*#__PURE__*/function(){function Immutable(input){var _this=this;_classCallCheck(this,Immutable);Object.keys(input).forEach(function(key){if(is.array(input[key])){_this[key]=freezeArray(input[key])}else if(is.object(input[key])){_this[key]=new Immutable(input[key])}else if(functionsOnPrototype&&is.function(input[key])){Immutable.prototype[key]=input[key]}else{_this[key]=input[key]}});if((this instanceof Immutable?this.constructor:void 0)===Immutable){Object.freeze(this)}}_createClass(Immutable,[{key:"toObject",value:function toObject(options){return _toObject(this,options)}}]);return Immutable}();/**
         * Validates, and then freezes an object, and all of it's values, recursively
         * @param {object} input - the object to freeze
         */var ValidatedImmutable=/*#__PURE__*/function(_Immutable){_inherits(ValidatedImmutable,_Immutable);function ValidatedImmutable(input){var _this2;_classCallCheck(this,ValidatedImmutable);var result=validator.validate(input);_this2=_possibleConstructorReturn(this,_getPrototypeOf(ValidatedImmutable).call(this,result&&result.value||input));if((this instanceof ValidatedImmutable?this.constructor:void 0)===ValidatedImmutable){Object.freeze(_assertThisInitialized(_this2))}return _this2}_createClass(ValidatedImmutable,[{key:"patch",value:function patch(input){return new ValidatedImmutable(_patch(this)(input))}},{key:"getSchema",value:function getSchema(){return schema}}]);return ValidatedImmutable}(Immutable);return ValidatedImmutable}}return{immutable:new ImmutableInstance,Immutable:ImmutableInstance,patch:_patch,array:function array(arr){return{push:push(arr),pop:pop(arr),shift:shift(arr),unshift:unshift(arr),sort:sort(arr),reverse:reverse(arr),splice:splice(arr),remove:remove(arr),copy:copy(arr)}}}}};root.polyn=root.polyn||{};if(!root.polyn.blueprint){throw new Error("@polyn/immutable requires @polyn/blueprint to be included before it")}var immutable=module.factories.immutable(root.polyn.blueprint);root.polyn.immutable=Object.freeze(_objectSpread({},immutable));// we don't need these anymore
