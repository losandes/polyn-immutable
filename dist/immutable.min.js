...
bp=name}else{bp=blueprint(name,schema)}if(bp.err){throw bp.err}return{validate:function validate(input){var validationResult=bp.validate(input);if(validationResult.err){throw validationResult.err}return validationResult}}}/**
     ...
     */var Immutable=/*#__PURE__*/function(){function Immutable(input){var _this=this;_classCallCheck(this,Immutable);Object.keys(input).forEach(function(key){if(is.array(input[key])){_this[key]=freezeArray(input[key])}else if(is.object(input[key])){_this[key]=new Immutable(input[key])}else{_this[key]=input[key]}});if((this instanceof Immutable?this.constructor:void 0)===Immutable){Object.freeze(this)}}_createClass(Immutable,[{key:"toObject",value:function toObject(options){return _toObject(this,options)}}]);return Immutable}();function ImmutableInstance(config){config=_objectSpread({},{Validator:Validator},config);/**
       ...
       * @param {object} schema - the blueprint schema
       */return function(name,schema){var validator=new config.Validator(name,schema);/**
