...
configurable:false});module.exports={name:"immutable",factory:function factory(Blueprint){'use strict';var is=Blueprint.is,blueprint=Blueprint.blueprint;/**
     ...
     * @param {any} input - the value to test
     */var isImmutable=function isImmutable(input){var proto=Object.getPrototypeOf(input);var result=is.object(input)&&(is.function(input.isPolynImmutable)||is.function(proto&&proto.isPolynImmutable));return result};/**
...
bp=name}else{bp=blueprint(name,schema)}if(bp.err){throw bp.err}return{validate:function validate(input){var validationResult=bp.validate(input);if(validationResult.err){throw validationResult.err}return validationResult}}}/**
     ...
     */var _toObject=function toObject(that,options){var shallowClone=Object.assign({},that);var output={};var _removeFunctions$opti=_objectSpread({},{removeFunctions:false},options),removeFunctions=_removeFunctions$opti.removeFunctions;Object.keys(shallowClone).forEach(function(key){if(shallowClone[key]&&typeof shallowClone[key].toObject==="function"){output[key]=shallowClone[key].toObject(options)}else if(is.array(shallowClone[key])){output[key]=Object.assign([],shallowClone[key])}else if(is.object(shallowClone[key])){output[key]=Object.assign({},shallowClone[key])}else if(is.function(shallowClone[key])&&removeFunctions===true){// do nothing
       ...
       * @param {object} schema - the blueprint schema
       */var immutable=function immutable(name,schema,options){var validator=new config.Validator(name,schema);var _functionsOnPrototype=_objectSpread({},{functionsOnPrototype:false},options),functionsOnPrototype=_functionsOnPrototype.functionsOnPrototype;// NOTE the classes, and freezeArray are in here, so their
